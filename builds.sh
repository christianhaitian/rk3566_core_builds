...
do
  if [[ "$var" != "all" ]] && [[ "$var" != "ALL" ]]; then
    ...
    if test -z "$(ls -l /usr/lib/${archtest}/libMali.so | grep g52-g29p1 | tr -d '\0')"
    then
      printf "\nUpdating libMali to properly link to ibmali-bifrost-g52-g29p1.so in the dev environment, please wait...\n"
      if [[ ${bitness} == "32" ]] && [[ ! " ${g31only[*]} " =~ " ${var} " ]]; then
        cp mali/armhf/libmali-bifrost-g52-g29p1.so /usr/lib/arm-linux-gnueabihf/.
        cp mali/armhf/libmali-bifrost-g52-g29p1.so /usr/local/lib/arm-linux-gnueabihf/.
        cd /usr/lib/arm-linux-gnueabihf/
        whichmali="libmali-bifrost-g52-g29p1.so"
        arch="arm-linux-gnueabihf"
      elif [[ ! " ${g31only[*]} " =~ " ${var} " ]]; then
        cp mali/aarch64/libmali-bifrost-g52-g29p1.so /usr/lib/aarch64-linux-gnu/.
        cp mali/aarch64/libmali-bifrost-g52-g29p1.so /usr/local/lib/aarch64-linux-gnu/.
      ...
      ln -sf libMali.so libMaliOpenCL.so.1
