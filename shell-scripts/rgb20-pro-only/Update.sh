...
elif [[ -e "/dev/input/by-path/platform-singleadc-joypad-event-joystick" ]] && [ "$(cat ~/.config/.DEVICE)" == "RGB30" ]; then
  wget -t 3 -T 60 --no-check-certificate "$LOCATION"/Update-RGB30.sh -O /home/ark/ArkOSUpdate.sh -a "$LOG_FILE"
elif [[ -e "/dev/input/by-path/platform-singleadc-joypad-event-joystick" ]] && [ "$(cat ~/.config/.DEVICE)" == "RGB20PRO" ]; then
  wget -t 3 -T 60 --no-check-certificate "$LOCATION"/Update-RGB20PRO.sh -O /home/ark/ArkOSUpdate.sh -a "$LOG_FILE"
