...
bitness="$(getconf LONG_BIT)"
	...
	if [[ "$var" == "mupen64plus-nx" ]]; then
	 ...
	 if [[ ! -z "$mupen64plus_core_patches" ]]; then
	  ...
	  if [[ "$bitness" == "64" ]]; then
	    make platform=arkos BOARD=NGRK -lto -j$(nproc)
	  else
	    make platform=arkos BOARD=NGRK32BIT -lto -j$(nproc)
	  fi
	  ...
	  echo " "
	  echo "mupen64plus_next_libretro.so executable has been created and has been placed in the rk3566_core_builds/cores$bitness subfolder"
