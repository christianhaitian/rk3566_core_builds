...
bitness="$(getconf LONG_BIT)"
	...
	if [[ "$var" == "mgba" || "$var" == "all" ]] && [[ "$bitness" == "64" ]]; then
	 ...
	 if [[ ! -z "$mgba_patches" ]]; then
	  ...
	  #sed -i '/-mcpu=cortex-a35/s//-mtune=cortex-a55/g' Makefile.libretro
	  ...
	  echo " "
	  echo "mgba_libretro.so and mgba_rumble_libretro.so has been created and has been placed in the rk3566_core_builds/cores64 subfolder"
	fi
