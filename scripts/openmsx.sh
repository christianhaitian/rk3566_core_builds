...
TAG="RELEASE_17_0"
	...
	if [[ "$var" == "openmsx" ]] && [[ "$bitness" == "64" ]]; then
	 ...
	 if [[ ! -z "$openmsx_patches" ]]; then
	  ...
	  echo " "
	  echo "The openmsx executable and openmsx_pkg_$(git rev-parse HEAD | cut -c -7).tar.gz package has been created and has been placed in the rk3566_core_builds/openmsx$bitness subfolder"
