	...
	if [[ "$var" == "hypseus" ]] && [[ "$bitness" == "64" ]]; then
	 ...
	 if [[ ! -z "$hypseus_patches" ]]; then
	  ...
	  do
	       ...
	       echo " "
	       echo "The hypseus.$(echo $patching | cut -d '-' -f 4 | cut -d '.' -f 1) executable has been created and has been placed in the rk3566_core_builds/hypseus$bitness subfolder"
