...
bitness="$(getconf LONG_BIT)"
	...
	if [[ "$var" == "scummvm-libretro" || "$var" == "all" ]] && [[ "$bitness" == "64" ]]; then
	 ...
	 if [[ ! -z "$scummvm_patches" ]]; then
	  ...
	  sed -i '/a53/s//a55/' backends/platform/libretro/Makefile
	  sed -i '/rpi3_64/s//rk3566/' backends/platform/libretro/Makefile
	  make -C backends/platform/libretro platform=rk3566 CXXFLAGS="$CXXFLAGS -DHAVE_POSIX_MEMALIGN=1" -j$(nproc)
