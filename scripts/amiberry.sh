...
bitness="$(getconf LONG_BIT)"
	...
	if [[ "$var" == "amiberry" ]] && [[ "$bitness" == "64" ]]; then
     ...
     sed -i '/RK3326/s//RK3566/g' Makefile

     make -j$(nproc) PLATFORM=RK3566
     if [[ $? != "0" ]]; then
       echo " "
	  ...
	  echo " "
	  echo "amiberry has been created and has been placed in the rk3566_core_builds/amiberry64 subfolder"
	fi
