...
bitness="$(getconf LONG_BIT)"
	...
	if [[ "$var" == "ppsspp-libretro" || "$var" == "all" ]] && [[ "$bitness" == "64" ]]; then
	 ...
	 if [[ ! -z "$ppsspp_patches" ]]; then
	  ...
	  mkdir build
	  cd build
	  cmake -DLIBRETRO=ON -DUSING_FBDEV=ON -DUSING_EGL=ON -DUSING_GLES2=ON \
	  -DUSE_FFMPEG=YES -DUSE_SYSTEM_FFMPEG=NO -DUSING_X11_VULKAN=OFF -DUSE_VULKAN_DISPLAY_KHR=OFF ../.
	  make -j$(nproc)
	  ...
	  echo " "
	  echo "ppsspp_libretro.so has been created and has been placed in the rk3566_core_builds/cores64 subfolder"
