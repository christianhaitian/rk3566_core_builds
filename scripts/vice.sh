...
bitness="$(getconf LONG_BIT)"
	...
	if [[ "$var" == "vice" || "$var" == "all" ]]; then
	 ...
	 if [[ $? != "0" ]]; then
	  ...
	  echo " "
	  echo "128 x64sc xscpu64 xplus4 xvic xcbm5x0 xcbm2 xpet and x64 vice_libretro cores have been created and has been placed in the rk3566_core_builds/cores$(getconf LONG_BIT) subfolder"
	fi
