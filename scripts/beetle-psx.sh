...
bitness="$(getconf LONG_BIT)"
	...
	if [[ "$var" == "beetle-psx" || "$var" == "all" ]] && [[ "$bitness" == "64" ]]; then
	 ...
	 if [[ ! -z "$beetle_psx_patches" ]]; then
	  ...
	  sed -i '/a72/s//a55/g' Makefile
	  sed -i '/rpi4_64/s//rk3566_64/' Makefile
	  #sed -i '/GLES \= 0/s//GLES \= 1/g' Makefile
	  make clean
	  make -j$(nproc)
	  make platform=rk3566_64 HAVE_HW=1 -j$(nproc)
	  ...
	  echo " "
	  echo "mednafen_psx_hw_libretro.so has been created and has been placed in the rk3566_core_builds/cores64 subfolder"
	fi
